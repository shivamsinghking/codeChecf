...
public class Main
{
    ...
    static int findInversion(String s, int m){
    ...
    static int[] findOneAndZero(String s){
      int z = 0;
      int one = 0;
      ...
      // out.println(z + " " + one + " " + s + " " + s.length());
      return new int[]{s.length() - one, one};
    }
    public static void solve()
    {
      ...
      for(int i = 0; i < n; i++){
        total += findInversion(arr[i], m);
        // out.println(" total " + total);
        count[i] = findOneAndZero(arr[i]);
      }

      // for(int[] i: count){
      //   out.println(Arrays.toString(i));
      // }
      // out.println(Arrays.toString(count) + " total => "  + total);
      // count => zero, one
      ...
      // for(int[] i: count){
      //   out.println(" -- cnt " + Arrays.toString(i));
      // }

      int[] countZero = new int[n];
      countZero[0] = count[0][0];
      for(int i = 1; i < n; i++){
        countZero[i] = count[i][0] + countZero[i-1];
      }

      for(int i = 0; i < n; i++){
        total += (count[i][1]*(countZero[n-1] - countZero[i]));
      }
